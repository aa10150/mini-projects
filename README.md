# Mini Projects by Anshu Aramandla

**ChemicalEquationBalancer:** uses linear algebra to balance chemical equations with support for subgroups (polyatomic ions, functional groups, etc.) and hydrate dot. Requires numpy and sympy packages.  
**MazeGame:** a 2D maze with four levels and three difficulty options. Requires Linux/WSL.  
**PokerHands:** input a set of 5 playing cards or generate one randomly and find its rank in poker.
**PolyrhythmPlayer:** PolyrhythmVisualizer with sound. Requires pygame-ce package.  
**PolyrhythmVisualizer:** a polyrhythm visualizer with accurate tempo.  
**Solitaire:** solitaire card game.  
**TicTacToe:** two-player tic tac toe. Requires Linux/WSL.